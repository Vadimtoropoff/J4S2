...
# regex s'écrit entre crochet et en / /
# \ : pour des raccourcis
# |x| : définir une variable qu'on utilise après
# \d is the same as [0-9] : on prend ceux qui ont un 0... 9 et seront donc comptabilisé dans le count
...
# http://zetcode.com/lang/rubytutorial/regex/ : on utilise la fonction.
# J'ai cherché tous les aude SANS majuscule
# x.match /aude/i ==> on prend tout en compte : majuscule et minuscule
...
# [/[A-Z]/] : on ne cherche que les majuscules dans les mots.
# on utilise slice pour trouver la lettre dans le mot. 
...
# On peut mettre avec ou sans crochets pour trouver, cela donne le meme résultat

puts "Trie la liste de handle par ordre alphabétique ?"
print ">"
puts table.sort_by(&:downcase)
# .sort permet de classer par ordre alphabétique (et va tout mettre en minscule pour chercher)

puts "Quels sont les 50 handles les plus courts de ce array ?"
print ">"

table_sorted = table.sort_by!{
  |x| 
  x.length
}
...
# D'abord je trie et ensuite je fais apparaitre 50 fois avec une boucle à l'intérieur du tableau. 
